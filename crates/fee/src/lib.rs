...
pub mod pallet {
        ...
        + currency::Config<UnsignedFixedPoint = UnsignedFixedPoint<Self>, SignedFixedPoint = SignedFixedPoint<Self>>
    {
        ...
        type CapacityRewards: RewardsApi<(), CurrencyId<Self>, CurrencyId<Self>, BalanceOf<Self>>;

        /// Vault reward pool.
        type VaultRewards: RewardsApi<(), DefaultVaultId<Self>, BalanceOf<Self>, CurrencyId = CurrencyId<Self>>;
...
impl<T: Config> Pallet<T> {
    ...
    fn withdraw_vault_rewards(
        ...
        let rewards = T::StakingRewards::withdraw_reward(&(index, vault_id.clone()), nominator_id, currency_id)?
            .try_into()
    ...
    fn distribute_vault_rewards(vault_id: &DefaultVaultId<T>, currency_id: CurrencyId<T>) -> DispatchResult {
        ...
        T::StakingRewards::distribute_reward(&(None, vault_id.clone()), currency_id, remainder.amount())?;
...
impl<T> RewardsApi<(), (DefaultVaultId<T>, T::AccountId, Option<T::Index>), CurrencyId<T>, BalanceOf<T>>
    for RewardsRouter<T>
where
    T: Config,
{
    ...
    fn deposit_stake(
        ...
        Pallet::<T>::distribute_all_vault_rewards(vault_id)?;
        // Deposit `amount` of stake in the pool
        T::StakingRewards::deposit_stake(&(*index, vault_id.clone()), nominator_id, amount)
    }
    ...
    fn withdraw_stake(
        ...
        Pallet::<T>::distribute_all_vault_rewards(vault_id)?;
        T::StakingRewards::withdraw_stake(&(*index, vault_id.clone()), nominator_id, amount)
    }
    ...
    fn get_stake(
        ...
        T::StakingRewards::get_stake(&(*index, vault_id.clone()), nominator_id)
    }
}
